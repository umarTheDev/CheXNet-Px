...
│   └── build_splits.py               # Train/val/test split generation
│
├── Model Architecture
│   └── model.py                      # DenseNet121Binary implementation
│
├── Training & Evaluation
...
│   └── xai.py                        # Explainability analysis (XAI)
│
├── Pipeline & Orchestration
│   └── run_pipeline.py               # End-to-end execution orchestrator
│
├── data/                             # Dataset directory
│   ├── images/                       # Chest X-ray images (not in repo)
│   ├── labels.txt                    # Master label file (24,999 images)
...
│   └── folds/                        # K-fold cross-validation splits
│
├── experiment-logs/                  # Training logs and experiment tracking
│
├── Checkpoints/                      # Saved model weights
│   ├── model.pth.tar                 # Pretrained CheXNet baseline
│   ├── best_binary_model.pth.tar     # Single-fold best checkpoint
│   ├── best_binary_fold0.pth.tar     # K-fold checkpoint - fold 0
│   ├── best_binary_fold1.pth.tar     # K-fold checkpoint - fold 1
│   ├── best_binary_fold2.pth.tar     # K-fold checkpoint - fold 2
│   └── calibration_params.pkl        # Platt scaling calibration parameters
│
...
python -c "from read_data import ChestXrayDataSet; print('✓ Installation successful')"
```
...
```bash
   ...
   mkdir -p data/images
   # Place all chest X-ray images in data/images/
```

2. **Create Label Files**
```bash
   # Create data/labels.txt with format: image_name label (0 or 1)
   # Example:
   # image_001.png 0
   # image_002.png 1
...
4. **Verify Dataset**
```bash
   python -c "from read_data import ChestXrayDataSet
...
# 2. Apply calibration
python calibrate.py \
  --model_path best_binary_model.pth.tar \
  --val_list data/val_list.txt \
...
Progress: 100%
├── ROC-AUC: ~0.89 (varies by fold/data)
├── PR-AUC:  ~0.74
├── Accuracy: ~0.91
├── Sensitivity: ~0.49
├── Specificity: ~0.86
...
**Note:** Sensitivity gap is attributable to smaller pneumonia training set. ROC-AUC and PR-AUC metrics demonstrate robust ranking-based decision-making.

### Detailed Results by Experiment

- **EXP-001:** Hardware compatibility achieved; baseline inference verified on Apple Silicon
- **EXP-002:** Frozen backbone achieved ROC-AUC ~0.83; established baseline reproduction
- **EXP-003:** Unfrozen backbone with calibration achieved ROC-AUC ~0.89; exceeded baseline

See checkpoint files (`best_binary_fold*.pth.tar`) and experiment logs in `experiment-logs/` directory for detailed analysis.
...
See `model.py` for the complete layer-by-layer implementation.
...
### EXP-001: Hardware Adaptation (Week 1-2)
**Goal:** Adapt code to Apple Silicon MPS backend
**Status:** ✓ Complete
**Key Changes:**
...
Details logged in `experiment-logs/` directory.

### EXP-002: Binary Classification (Week 3-4)
**Goal:** Reproduce baseline with frozen backbone
**Status:** ✓ Complete
**Results:** ROC-AUC ~0.83, achieved baseline reproduction target
**Key Features:**
...
**Status:** ✓ Complete
**Results:** ROC-AUC ~0.89, exceeded baseline
**Key Improvements:**
...
- Hyperparameters documented in script docstrings and code comments
- All experiments logged with timestamps in `experiment-logs/`
...
For additional issues, refer to specific script docstrings or check experiment logs in `experiment-logs/` directory.
...
This repository is part of an academic assignment. If you're building upon this work:
1. Maintain proper attribution to the baseline paper (Yassen 2025)
2. Document any modifications or extensions
3. Report metrics with confidence intervals
4. Include reproducibility details (seed, device, data splits)
